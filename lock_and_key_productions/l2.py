...
class L2():
    ...
    def apply(G: nx.Graph, isomorphisms: List[Dict] = None):
        ...
        lock = list(n for n,_ in isomorphism.items() if G.nodes[n]['type'] == 'lock')[0]
        ...
        G.remove_edges_from([(access, lock)])
        Access.add_access_edge(G, new_access, lock)
        return True
